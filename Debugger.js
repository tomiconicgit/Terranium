...
export class Debugger {
  ...
  /**
   * Quick on-demand check of module URLs. We’ll GET with cache-busting and report non-200s.
  ...
  async verifyModuleList(urls = []) {
    ...
    const lines = results.map(r => `${r.ok ? '✅' : '⛔️'} ${r.url}  [${r.status}${r.type ? ' '+r.type : ''}]`);
  ...
  _onWindowError(evt) {
    ...
    if (message && String(message).includes('Importing a module script failed')) {
      // This is the generic ESM load failure—trigger our analyzer with extra context.
