// src/ui/ModelSliders.js — Updated for extended + exponential scale range
...
export class ModelSlidersUI {
  ...
  updateSlidersFromModel() {
    ...
    this.sliders.scale.value = Math.cbrt(scale); // reverse of exponential mapping
    this.updateValueLabels(scale);
  }
  ...
  updateModelTransform() {
    ...
    // Exponential mapping for finer low-end precision
    const raw = parseFloat(this.sliders.scale.value);
    const scale = Math.pow(raw, 3); // cubic mapping gives smooth fine control

    this.activeModel.scale.set(scale, scale, scale);
    this.updateValueLabels(scale);
  }

  updateValueLabels(scaleOverride = null) {
    ...
    const scale = scaleOverride ?? Math.pow(parseFloat(this.sliders.scale.value), 3);
    this.valueLabels.scale.textContent = scale.toFixed(3);
  }
