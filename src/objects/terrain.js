...
export function createTerrain() {
  ...
  for (let i = 0; i < vertices.length; i += 3) {
    const x = vertices[i] / width + 0.5;
    const y = vertices[i + 1] / height + 0.5;
    vertices[i + 2] = -fbm(x * 5, y * 5, 6) * 10;
  }
  ...
  // Moondusted
  const map1 = loader.load('src/assets/textures/moon/moondusted/moondusted-diffuse.jpg');
  map1.wrapS = map1.wrapT = THREE.RepeatWrapping; map1.repeat.set(5, 5);

  const displacement1 = loader.load('src/assets/textures/moon/moondusted/moondusted-displacement.png');
  displacement1.wrapS = displacement1.wrapT = THREE.RepeatWrapping; displacement1.repeat.set(5, 5);

  const normalMap1 = exrLoader.load('src/assets/textures/moon/moondusted/moondusted-normal.exr');
  normalMap1.wrapS = normalMap1.wrapT = THREE.RepeatWrapping; normalMap1.repeat.set(5, 5);

  const roughnessMap1 = exrLoader.load('src/assets/textures/moon/moondusted/moondusted-roughness.exr');
  roughnessMap1.wrapS = roughnessMap1.wrapT = THREE.RepeatWrapping; roughnessMap1.repeat.set(5, 5);

  // Moonflatmacro
  const map2 = loader.load('src/assets/textures/moon/moonflatmacro/moonflatmacro-diffuse.jpg');
  map2.wrapS = map2.wrapT = THREE.RepeatWrapping; map2.repeat.set(5, 5);

  const normalMap2 = exrLoader.load('src/assets/textures/moon/moonflatmacro/moonflatmacro-normal.exr');
  normalMap2.wrapS = normalMap2.wrapT = THREE.RepeatWrapping; normalMap2.repeat.set(5, 5);

  const roughnessMap2 = exrLoader.load('src/assets/textures/moon/moonflatmacro/moonflatmacro-roughness.exr');
  roughnessMap2.wrapS = roughnessMap2.wrapT = THREE.RepeatWrapping; roughnessMap2.repeat.set(5, 5);

  // Moonnormal
  const map3 = loader.load('src/assets/textures/moon/moonnormal/moonnormal-diffuse.jpg');
  map3.wrapS = map3.wrapT = THREE.RepeatWrapping; map3.repeat.set(5, 5);

  const normalMap3 = exrLoader.load('src/assets/textures/moon/moonnormal/moonnormal-normal.exr');
  normalMap3.wrapS = normalMap3.wrapT = THREE.RepeatWrapping; normalMap3.repeat.set(5, 5);

  const roughnessMap3 = exrLoader.load('src/assets/textures/moon/moonnormal/moonnormal-roughness.exr');
  roughnessMap3.wrapS = roughnessMap3.wrapT = THREE.RepeatWrapping; roughnessMap3.repeat.set(5, 5);
  ...
  material.onBeforeCompile = (shader) => {
    ...
    const noiseGlsl = `
      ...
      float fbm(vec2 xy){
        ...
        float frequency = 1.0;
        for (int i = 0; i < 4; i++){
          total += amplitude * noise2d(xy * frequency);
          amplitude *= 0.5;
          frequency *= 2.0;
        }
        return total;
  ...
  terrain.receiveShadow = true; // <- important
  return terrain;
}