...
export function createProps({ areaSize = 100, avoidRadius = 12, rockCount = 150, treeCount = 120, uniformsRef } = {}) {
    ...
    // --- Rocks (icosahedron with slight vertex jitter) ---
    const rockGeom = new THREE.IcosahedronGeometry(0.7, 0);
    jitterGeometry(rockGeom, 0.12);
    ...
    // --- Trees (stylized pine: trunk + 3 stacked canopies), with wind sway ---
    const trunkG = new THREE.CylinderGeometry(0.12, 0.16, 1.6, 6);
    ...
    // Wind sway via vertex transform in shader (uses shared uniformsRef.time)
    ...
    treeMat.onBeforeCompile = (shader) => {
        ...
        `.replace(
            ...
            `
            #include <begin_vertex>
            // sway more at canopy (y higher), less at trunk base
            float sway = 0.02 + 0.02 * sin( (position.y + modelMatrix[3].x + modelMatrix[3].z) * 0.8 + time*1.5 );
            transformed.x += sway * (position.y * 0.6);
            transformed.z += sway * (position.y * 0.6) * 0.6;
            `
        );
    };
    ...
    // Tick list (kept for possible future per-frame logic)
    const tickers = [ (dt)=>{} ];

    return { rocks, trees, tickers };
}

function jitterGeometry(geom, amt=0.1) {
...
// Minimal merge (keeps this file standalone)
