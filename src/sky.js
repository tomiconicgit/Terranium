...
export function createSky(scene, renderer) {
  ...
  // --- Separate SKY exposure: patch shader once, add uniform ---
  const skyExposureUniform = { value: 1.0 };
  sky.material.onBeforeCompile = (shader) => {
    shader.uniforms.uSkyExposure = skyExposureUniform;
    // Multiply final sky color by uSkyExposure
  ...
  // Global tone map still available if you want it,
  // but you can keep this constant now that we have split controls.
  ...
  const starMat = new THREE.ShaderMaterial({
    transparent:true, depthWrite:false, uniforms:starUniforms,
    vertexShader: `
  ...
  // --- Store base light intensities for "terrain light exposure" scaling ---
  ...
  const api = {
    ...
    setLightingExposure(v){
      // scale both lights together; keep ambient gentler so it doesn’t wash shadows
      sunDir.intensity = baseSun * v;
      ambient.intensity = baseAmb * Math.pow(v, 0.75);
    },

    // (keep global if you still want it — optional)
    setExposureGlobal(v){ renderer.toneMappingExposure = v; },

    // lighting color/intensity direct tweaks still allowed
